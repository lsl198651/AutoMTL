...
class SuperNet(BasicNetwork):
    def __init__(
        ...
        # TODO 𝑴𝒙𝒕𝒖𝒓𝒆 𝒐𝒇 𝑶𝒑𝒆𝒓𝒂𝒕𝒊𝒐𝒐𝒏𝒔 𝒇𝒐𝒓 𝑬𝒙𝒑𝒆𝒓𝒕𝑴𝒐𝒅𝒖𝒍𝒆
    ...
    #     mix_features = [feature_module(embs, dense_fea) for feature_module in self.feature_modules]
    #     mix_features.append(
    #         nn.functional.pad(embs.view(embs.size(0), -1),
    #                           (0, self.embedding_dim + self.embedding.n_dense)))  # raw feature input for gate
    #
    #     temp = []
    ...
    def forward(self, x, logmel):
        # 处理logmel谱
        logmel_processed = self.logmel_linear(logmel)  # [B, logmel_dim] -> [B, embedding_dim]

        # 处理嵌入特征
        embs, dense_fea = self.embedding(x, self.features, squeeze_dim=False)  # [B, N, E], [B, n_dense_fields]
        embs = torch.cat([embs, logmel_processed.unsqueeze(1)], dim=1)  # 将logmel谱加到嵌入特征中
    ...
    def compute_log_mel_spectrogram(audio, sample_rate=16000, n_fft=1024, hop_length=512, n_mels=128):
        mel_spectrogram = T.MelSpectrogram(sample_rate=sample_rate, n_fft=n_fft, hop_length=hop_length, n_mels=n_mels)(
            audio)
        log_mel_spectrogram = torch.log(mel_spectrogram + 1e-10)  # Add a small constant to avoid log(0)
